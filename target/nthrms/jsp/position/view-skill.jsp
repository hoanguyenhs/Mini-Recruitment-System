<script>
	...
	$(document).ready(function(){
		...
		$.ajax({
 		...
 		$.ajax({
	 		type: "GET",
	 		url: "${pageContext.request.contextPath}/position/getSkill/${positionModel.id }",
	 		async: false,
	 		success: function(data) {
	 			$.each( data, function( key, val ) {
 		...
 		for(var i = 0; i < skillList.length; i++){
 	 		for(var j = 0; j < skillListDB.length; j++){
				if(skillList[i].name == skillListDB[j].name){
					...
					skillList[i].idPosition = skillListDB[j].idPosition;
				}
 	 		}
 		}
		...
		for(var i = skillList.length-1; i > -1; i--) {
 		...
 		for(var i = 0; i < skillDomain.length; i++) {
 	 		$(".nth-skill-domain").after("<tr>"
 	 	 		...
 	 	 		+"</td>"
 	 	 	 	+"<td><input type='radio' value='1' name='skillDomain["+i+"]' disabled/></td>"
 	 	 	 	+"<td><input type='radio' value='2' name='skillDomain["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='3' name='skillDomain["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='4' name='skillDomain["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='5' name='skillDomain["+i+"]' disabled/></td>"
 	 	 	 	+"</tr>");
 		...
 		for(var i = 0; i < skillTechnical.length; i++) {
 	 		$(".nth-skill-technical").after("<tr>"
	 			+"<td>"+skillTechnical[i].name
 	 	 		...
 	 	 		+"</td>"
 	 	 	 	+"<td><input type='radio' value='1' name='skillTechnical["+i+"]' disabled/></td>"
 	 	 	 	+"<td><input type='radio' value='2' name='skillTechnical["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='3' name='skillTechnical["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='4' name='skillTechnical["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='5' name='skillTechnical["+i+"]' disabled/></td>"
 	 	 	 	+"</tr>");
 		...
 		for(var i = 0; i < skillLanguage.length; i++) {
 	 		$(".nth-skill-language").after("<tr>"
 	 			+"<td>"+skillLanguage[i].name
 	 	 		...
 	 	 		+"</td>"
 	 	 	 	+"<td><input type='radio' value='1' name='skillLanguage["+i+"]' disabled/></td>"
 	 	 	 	+"<td><input type='radio' value='2' name='skillLanguage["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='3' name='skillLanguage["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='4' name='skillLanguage["+i+"]' disabled/></td>"
 	 	 		+"<td><input type='radio' value='5' name='skillLanguage["+i+"]' disabled/></td>"
 	 	 	 	+"</tr>");
